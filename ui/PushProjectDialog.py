...
class PushProjectDialog(QDialog, FORM_CLASS):
    ...
    def save_all_layers(self):
        ...
        before pushing to MapHub, preventing data loss or inconsistencies
        between local files and what gets pushed to the server.
        """
        ...
        # Save each editable vector layer
        for layer in layers:
            if isinstance(layer, QgsVectorLayer) and layer.isEditable():
                print(f"Saving changes to layer: {layer.name()}")
                layer.commitChanges()
